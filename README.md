...
`FlickFit_v1.0.1.zip` を好きな場所に展開します。
...
GitHub からダウンロードした ZIP / BAT / PowerShell スクリプトは、Windows により「インターネットから入手したファイル」として扱われ、初回実行時に警告が出ることがあります。

内容を確認し、信頼できる場合のみ実行してください。

警告を解除したい場合は、ZIP を展開する前に ZIP ファイルを右クリックし、`プロパティ` → `許可する`（または `ブロックの解除`）にチェックしてから展開してください。
...
次はローカル環境や実行履歴が混ざりやすいため、公開配布物からは除くことを推奨します。

```text
*.pyc
__pycache__/
.jxl-plugin-cache.json
launcher_trace.log
VolumePatternRules.local.txt
.vscode/
temp/
```
...
Copyright (c) 2026 jaijaijai20060814
